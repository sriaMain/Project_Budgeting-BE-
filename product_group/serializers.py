...
class QuoteSerializer(serializers.ModelSerializer):
	...
	# Restrict one quotation to one project
		from Project.models import Project
		quote_id = self.instance.id if self.instance else None
		if quote_id:
			project_exists = Project.objects.filter(created_from_quotation=quote_id).exists()
			if project_exists:
				raise serializers.ValidationError({
					"error": "A project already exists for this quotation. Only one project can be created per quotation."
				})
	...
	# 1️⃣ Enforce valid items only on CREATE
		...
		if self.instance:
			...
			# Once Cancelled, cannot move to any other status
			if current_status == 'Cancelled' and new_status != 'Cancelled':
				raise serializers.ValidationError({
					"error": (
						"Once a quotation is Cancelled, it cannot be moved to another status."
					)
				})
			# Once Rejected, can only move to Cancelled
			if current_status == 'Rejected' and new_status not in ['Rejected', 'Cancelled','Closed']:
				raise serializers.ValidationError({
					"error": (
						"Once a quotation is Rejected, it can only be moved to Cancelled."
					)
				})
			...
			if current_status == 'Closed' and new_status != 'Closed':
				raise serializers.ValidationError({
					"error": (
						"Once a quotation is Closed, it cannot be moved to another status."
					)
				})
